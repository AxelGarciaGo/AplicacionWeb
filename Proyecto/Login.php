...
// se inicializa la condición if que verifica si es que el boton de iniciar a sido  seleccionado
	if (isset($_POST['submit'])) {
		...
		if($usuario=='Admin' && $clave=='1'){//se agregan los valores de la base de datos para hacer conexión
			$servidor="localhost";
			$usuario="root";
			$contra="";
			...
			if ($consulta) {
				...
				if($consulta){
					...
					$db="proyecto";
					$conexion=mysqli_connect($servidor,$usuario,$contra,$db);
					$crearhabitantes="CREATE TABLE habitantes (num_habitante INT AUTO_INCREMENT UNIQUE, num_toma INT , 
